...
const errorList: Record<ErrorCode, IErrorDetails> = {
  ...
  [ErrorCode.TOKEN_EXPIRED]: { status: 403, description: 'Token expired' },
  [ErrorCode.SESSION_EXPIRED]: { status: 403, description: 'Session(refresh token) expired' },
  ...
  [ErrorCode.CONFLICT]: {
    status: 409,
    description: 'Duplicate conflict. Resource already exists. Try to use another one',
  },
...
export class AppError extends Error {
  ...
  constructor(
    ...
    super(options.message || errorConfig.description);
    ...
    this.status = options.status || errorConfig.status || 500;
